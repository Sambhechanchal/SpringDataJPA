...
public class JobSeekerServiceImpl implements IJobSeekerService {
	...
	public String deleteJsByIdsInBatch(List<Integer> ids) {
		List<JobSeeker> listJs = jobRepo.findAllById(ids);
		jobRepo.deleteAllByIdInBatch(ids);
		return listJs.size()+" records are deleted ";
	}
	@Override
	public List<JobSeeker> showJobSeekerByExampleData(JobSeeker Jexample, boolean ascOrder, String... props) {
		...
		Example example = Example.of(Jexample);
		List<JobSeeker> list = jobRepo.findAll(example, sort);
		return list;
	}
	
	@Override
	public JobSeeker findJobSeekerById(Integer id) {
		JobSeeker job = jobRepo.getReferenceById(id);
		return job;
	}
